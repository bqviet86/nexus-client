...
function PostForm({
    ...
    useEffect(() => {
        if (isOpenForm) {
            setContent(post?.content || '<br>')
            setMedias((prevMedias) => (!prevMedias.length ? post?.medias || [] : prevMedias))
            setIsDisabledSubmit(formType !== 'share_post')
        }
    }, [isOpenForm])
    ...
    const handleUploadMedias = async () => {
        const mediasWithFile = medias.filter((media) => (media as MediaWithFile).file) as MediaWithFile[]
    ...
    const handleSubmit = async () => {
        ...
        if (formType === 'edit_post') {
            ...
            const reqData: UpdatePostReqData = {
                content,
                hashtags,
                medias: (medias.filter((media) => !(media as MediaWithFile).file) as Media[]).concat(mediasRes)
            }
